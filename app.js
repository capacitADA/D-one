...
function goTo(view, id = null, eid = null) {
    ...
    document.querySelectorAll('.bni').forEach(b => {
        b.classList.toggle('active', b.dataset.page === view ||
            (view === 'detalleCliente' && b.dataset.page === 'clientes') ||
            (view === 'detalleTienda' && b.dataset.page === 'tiendas') ||
...
function renderTiendas() {
    return `<div class="page">
        ...
        <div id="tiendasGrid">
            ${tiendas.map(t => `
            <div class="cc" data-search="${(t.nombre+t.ciudad+t.telefono).toLowerCase()}">
                <div style="display:flex;justify-content:space-between;">
                    <div class="cc-name">${t.nombre}</div>
                    ${esAdmin() ? `<div><button class="ib" onclick="modalEditarTienda('${t.id}')">✏️</button><button class="ib" onclick="modalEliminarTienda('${t.id}')">🗑️</button></div>` : ''}
                </div>
...
function renderDetalleTienda() {
    ...
    return `<div class="page">
        <div class="det-hdr"><button class="back" onclick="goTo('tiendas')">← Volver</button><div><div class="cc-name">${t.nombre}</div><div class="cc-meta">${t.ciudad}</div></div></div>
        <div class="info-box">
            <div class="cc-row">📞 <strong>${t.telefono}</strong></div>
            <div class="cc-row">📍 ${t.direccion}</div>
            ${t.latitud ? `<a class="map-link" href="https://maps.google.com/?q=${t.latitud},${t.longitud}" target="_blank">🗺️ Ver en Google Maps</a>` : '<div class="cc-meta">Sin GPS</div>'}
        </div>
        ...
        <div class="ec">
            ...
            <div class="ec-btns">
                <button class="ab" onclick="goTo('historial','${t.id}','${e.id}')">📋 Servicios</button>
                <button class="ab" onclick="modalNuevoServicio('${e.id}')">➕ Nuevo</button>
                <button class="ab" onclick="generarInformePDF('${e.id}')">📄 PDF</button>
                <button class="ab" onclick="modalQR('${e.id}')">📱 QR</button>
            </div>
...
async function guardarCliente() {
    ...
    if (!sap || !nombre || !ciudad || !direccion || !coordinador || !cargo || !telefono) {
        toast('⚠️ Complete todos los campos obligatorios');
        return;
    }
    try {
        await addDoc(collection(db, 'clientes'), {
            sap, nombre, ciudad, departamento: depto, direccion, coordinador, cargo, telefono,
            email: email || '', latitud: lat, longitud: lng
        });
        closeModal();
        await cargarDatos();
        toast('✅ CEDI guardado');
...
async function actualizarCliente(cid) {
    ...
    try {
        await updateDoc(doc(db, 'clientes', cid), data);
        closeModal();
        await cargarDatos();
...
async function actualizarTienda(tid) {
    ...
    try {
        await updateDoc(doc(db, 'tiendas', tid), data);
        closeModal();
        await cargarDatos();
...
function modalEliminarTienda(tid) {
    if (!confirm('¿Eliminar esta Tienda y todos sus activos/servicios?')) return;
    eliminarTienda(tid);
}
async function eliminarTienda(tid) {
    ...
    try {
        ...
        await deleteDoc(doc(db, 'tiendas', tid));
        await cargarDatos();
        goTo('tiendas');
    ...
    showModal(`<div class="modal" onclick="event.stopPropagation()">
        ...
        <div class="modal-b">
            <div style="background:var(--bg2);padding:0.55rem;border-radius:8px;margin-bottom:0.65rem;">
                <strong>${ent?.nombre || 'Sin entidad'}</strong><br>
                <span style="font-size:0.75rem;">${e?.marca} ${e?.tipo || ''} ${e?.modelo}</span>
            </div>
            <div class="fr">
            ...
            <div class="reparacion-box hidden" id="reparacionBox">
                ...
                <select class="fi" id="estadoReparacion"><option value="OPERATIVO">OPERATIVO</option><option value="FUERA DE SERVICIO">FUERA DE SERVICIO</option><option value="DAR DE BAJA">DAR DE BAJA</option></select>
            </div>
            <label class="fl">📷 Fotos (max 3)</label>
            <div class="foto-row">${[0,1,2].map(i => `<div style="flex:1;"><div class="fslot" id="fslot${i}" onclick="document.getElementById('finput${i}').click()"><div class="fslot-plus">+</div><div class="fslot-lbl">Foto ${i+1}</div><input type="file" id="finput${i}" accept="image/*" style="display:none" onchange="previewFoto(this,${i})"></div></div>`).join('')}</div>
...
async function guardarServicio(eid) {
    ...
    try {
        await addDoc(collection(db, 'servicios'), {
            ...
            descripcion: desc, proximoMantenimiento: prox, fotos: fotosBase64, estadoReparacion: estadoRep
        });
        closeModal();
        await cargarDatos();
        const e = getEq(eid);
        if(e) goTo('historial', null, eid);
...
function generarInformePDF(eid) {
    ...
    <style>
        ...
        .coordinador{font-size:14px;font-weight:bold;color:#d10000;}
        .titulo{font-size:18px;font-weight:bold;margin-top:5px;}
    </style>
    </head><body>
    <div class="header">
        <div class="coordinador">Coordinador: ${coordinador} | Tel: ${telefonoCoord}</div>
        <div class="titulo">INFORME TECNICO</div>
    </div>
    <table style="width:100%;border-collapse:collapse;margin-bottom:12px;">
        <tr><td style="padding:6px;background:#f1f5f9;border:1px solid #ddd;"><strong>Cliente:</strong> ${ent?.nombre || 'N/A'}</td>
        <td style="padding:6px;background:#f1f5f9;border:1px solid #ddd;"><strong>Generado:</strong> ${new Date().toLocaleString()}</td></tr>
        <tr><td colspan="2" style="padding:6px;border:1px solid #ddd;"><strong>Activo:</strong> ${e?.marca || ''} ${e?.tipo || ''} ${e?.modelo || ''} &nbsp;&nbsp; <strong>Serial:</strong> ${e?.serie || 'N/A'}</td></tr>
    </table>
    <div style="background:#d10000;color:white;font-weight:bold;padding:6px;margin-bottom:10px;">HISTORIAL DE SERVICIOS (${ss.length})</div>
    ${serviciosHTML}
    </body></html>`;
    ...
    setTimeout(() => {
        ...
        logoImg.onload = () => {
            qrImg.onload = () => {
                ...
                ctx.font = '12px Arial';
                ctx.fillStyle = '#444';
                ctx.fillText('📍 ' + (e?.ubicacion || 'Sin ubicación'), W/2, y + 34);
                ctx.fillText('👤 ' + (ent?.nombre || ''), W/2, y + 50);
...
function manejarRutaQR() {
    ...
    const coordinador = ent?.coordinador || 'Coordinador';
    const telefono = ent?.telefono || '3239454477';
    const waMsg = encodeURIComponent(`Hola ${coordinador}, necesito ayuda con el equipo ${e?.marca || ''} ${e?.tipo || ''} ${e?.modelo || ''} de la ubicación ${e?.ubicacion || 'sin ubicación'}, podrías devolverme el mensaje`);
    const waUrl = `https://wa.me/57${telefono.replace(/\D/g,'')}?text=${waMsg}`;
    main.innerHTML = `<div style="max-width:600px;margin:0 auto;padding:1.5rem;">
        <div style="text-align:center;margin-bottom:0.75rem;"><img src="https://raw.githubusercontent.com/capacitADA/D-one/main/D1_logo.png" style="height:56px;" onerror="this.style.display='none'"></div>
        <div style="border:1px solid #ccc;border-radius:12px;padding:1rem;margin-bottom:0.75rem;">
            ...
            <p style="margin:2px 0;">📍 ${e.ubicacion || 'Sin ubicación'}</p>
            <p style="margin:2px 0;">👤 ${ent?.nombre || 'Sin entidad'}</p>
...
function renderTecnicos() {
    return `<div class="page">
        ...
        ${esAdmin() ? `<div style="margin-top:1.2rem;background:white;border-radius:12px;padding:0.85rem;">
            <div style="font-weight:700;">🏪 Subida Masiva de CEDIs y Tiendas</div>
            <div class="ec-meta">Sube un CSV con columnas: SAP, TIENDA, CIUDAD, DEPARTAMENTO, DIRECCION, COORDINADOR, CARGO, TELEFONO</div>
...
async function guardarTecnico() {
    ...
    try {
        await addDoc(collection(db, 'tecnicos'), {
            nombre: n, cedula: cc, tipoDoc: document.getElementById('tTipoDoc')?.value || 'CC',
            telefono: document.getElementById('tTel')?.value || '', cargo: document.getElementById('tCargo')?.value || '',
            rol: document.getElementById('tRol')?.value || 'tecnico', especialidades: [], region: '', clave: cl
        });
        closeModal();
        await cargarDatos();
...
function modalRecordar(clienteId, equipoId, fecha) {
    ...
    if (ent) {
        tel = ent.telefono;
        destinatario = `${ent.coordinador} · SAP ${ent.sap}`;
        msg = `Hola *${ent.coordinador}*, recordatorio: activo *${e?.marca} ${e?.tipo || ''} ${e?.modelo}* (${ent.tipoEntidad === 'cliente' ? 'CEDI' : 'Tienda'} ${ent.nombre}) requiere mantenimiento el *${fechaF}*. Confirmar visita. Coordinador Mtto 📞 3239454477`;
    } else {
        tel = '3239454477';
        destinatario = 'Coordinador';
        msg = `Recordatorio: activo *${e?.marca} ${e?.modelo}* requiere mantenimiento el *${fechaF}*.`;
    }
...
function aplicarFiltros() {
    ...
    el.innerHTML = filtrados.map(s => {
        ...
        return `<div class="si">
            ...
            <div class="si-info">👤 ${ent?.nombre||'N/A'} · ${e?.marca||''} ${e?.tipo||''} ${e?.modelo||''}</div>
...
function obtenerGPS(latId, lngId) {
...
async function subirCSVJMC(input) {
    ...
    reader.onload = async ev => {
        ...
        const encabezados = lines[0].split(',').map(h => h.trim().toUpperCase());
        const idxSap = encabezados.indexOf('SAP');
        const idxTienda = encabezados.indexOf('TIENDA');
        const idxCiudad = encabezados.indexOf('CIUDAD');
        const idxDepto = encabezados.indexOf('DEPARTAMENTO');
        const idxDir = encabezados.indexOf('DIRECCION');
        const idxCoord = encabezados.indexOf('COORDINADOR');
        const idxCargo = encabezados.indexOf('CARGO');
        const idxTel = encabezados.indexOf('TELEFONO');
        if (idxSap === -1 || idxTienda === -1) { toast('⚠️ El CSV debe tener columnas SAP y TIENDA'); return; }
        const cedis = [];
        const tiendasCSV = [];
        for (let i = 1; i < lines.length; i++) {
            const cols = lines[i].split(',').map(c => c.trim().replace(/^"|"$/g, ''));
            if (cols.length < Math.max(idxSap, idxTienda, idxCiudad, idxDepto, idxDir, idxCoord, idxCargo, idxTel)+1) continue;
            const sap = cols[idxSap];
            const nombre = cols[idxTienda];
            const ciudad = idxCiudad !== -1 ? cols[idxCiudad] : '';
            const departamento = idxDepto !== -1 ? cols[idxDepto] : '';
            const direccion = idxDir !== -1 ? cols[idxDir] : '';
            const coordinador = idxCoord !== -1 ? cols[idxCoord] : '';
            const cargo = idxCargo !== -1 ? cols[idxCargo] : '';
            const telefono = idxTel !== -1 ? cols[idxTel] : '';
            if (!sap || !nombre) continue;
            const item = { sap, nombre, ciudad, departamento, direccion, coordinador, cargo, telefono, latitud: '', longitud: '' };
            if (nombre.toUpperCase().includes('CEDI')) {
                cedis.push(item);
            } else {
                tiendasCSV.push(item);
            }
        }
        ...
        input.value = '';
        await cargarDatos();
        toast(`✅ ${cedis.length} CEDIs y ${tiendasCSV.length} Tiendas guardadas`);
    };
    reader.readAsText(file, 'UTF-8');
}
function descargarPlantillaCSV() {
    const enc = 'SAP,TIENDA,CIUDAD,DEPARTAMENTO,DIRECCION,COORDINADOR,CARGO,TELEFONO';
    const ejemplo = '170,Chia - Centro - Calle 13,Chia,Cundinamarca,Calle 13 # 9-43,Edgar Amado,Coordinador Sr Mantenimiento,3107935104';
    const csv = [enc, ejemplo].join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'plantilla_cedis_tiendas.csv';
    a.click();
